...
public:
  ...
  struct CustomCmp {
    bool operator()(
        ...
        const typename std::pair<std::weak_ptr<Node>, std::shared_ptr<E>>&right) const
        {
    ...
    public:
      ...
      reference operator*() const {
        // std::cout << " --- " << outer_->first << " "
        //           << inner_->first.lock()->getVal() << " " << *(inner_->second)
        //           << std::endl;
        return {outer_->first, inner_->first.lock()->getVal(), *(inner_->second)};
      }
    ...
    private:
      ...
      const typename std::map<N, std::shared_ptr<Node>>::const_reverse_iterator
          sentinel_;
      typename std::vector<std::pair<std::weak_ptr<Node>, std::shared_ptr<E>>>::const_reverse_iterator inner_;
